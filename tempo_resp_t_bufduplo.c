...
static void tempo_resp_t_insere_dado_arq(int dado){
    dados_arq = fopen("tempo_resp_t.txt", "a");
    if(dados_arq == NULL){
...
void tempo_resp_t_bufduplo_insereLeitura( double leitura){
...
void *tempo_resp_t_bufduplo_esperaBufferCheio(void) {
    double *buffer;
    pthread_mutex_lock( &exclusao_mutua);
    while( gravar == -1)
    ...
    for( int i=0; i<TAMBUF; ++i){
        tempo_resp_t_insere_dado_arq(buffer[i]);
    }

    pthread_create(&buffer_t, NULL, (void *) tempo_resp_t_bufduplo_esperaBufferCheio, NULL);
